...
struct SearchLimits {
    ...
    const std::atomic<bool>* stop{nullptr};  // External stop signal (UCI "stop")
};
...
private:
    ...
    std::vector<uint64_t> m_hashes{};  // Positions before the current one
    size_t m_root_index{0};            // Entries before this index are game history
    InfoCallback m_on_iteration{};

    template <bool Prune>
    int reference_search(Position& pos, int alpha, int beta, int depth, int ply) noexcept;
